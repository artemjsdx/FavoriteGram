#!/usr/bin/env python3
  ...
  # --- 8. FIX extractNativeLibs (CRITICAL: fixes crash on launch) ---
  def fix_extract_native_libs():
      ...
      # Try to add inside existing packagingOptions / packaging block
      patched = re.sub(
      ...
      # If no packagingOptions block found, add one before the closing android { } brace
      if patched == txt:
          patched = re.sub(
              r'(android\s*\{)',
              r'\1\n    packagingOptions {\n        useLegacyPackaging true\n    }',
